...
import ResourceSideBarWrapper from "./resourceSidebar.style";

const Sidebar = ( ) => {
  const data = useStaticQuery(
    graphql`
            query allResourceTagsAndCategories {
                tags: allMdx(
        ...
        <ul>
          { categories && categories.map(category => (
            <li key={category.fieldValue}>
              <Link to={`/resources/category/${slugify(category.fieldValue)}`}>
        ...
        <ul>
          { tags && tags.map(tag => (
            <li key={tag.fieldValue}>
              <Link to={`/resources/tag/${slugify(tag.fieldValue)}`}>{tag.fieldValue} ({tag.totalCount})</Link>
            </li>
          ))}
        </ul>
      </div>
