...
import { activityBadges, projectBadges, specialBadges as specialFromData } from "./badges-data";
...
const table = { width: "100%", borderCollapse: "collapse", marginBottom: "2rem" };
const theadRow = { borderBottom: "2px solid var(--text-color, #ddd)", backgroundColor: "rgba(0, 0, 0, 0.05)" };
const th = { textAlign: "left", padding: "12px", fontWeight: 600 };
...
const RecognitionPage = () => {
  const activity = (activityBadges || []).map(normalizeBadge);
  const projects = (projectBadges || []).map(normalizeBadge);
  const specials = (specialFromData || []).map(normalizeBadge);
      ...
      <div className="page-section">
        <Container>
          <div className="content">
            ...
            <p>
              Once a contributor joins the Layer5 community by filling in the community member form, they are automatically classified as
              members of Layer5. Although, you earn a member profile after consistent contributions to the community for and projects. You
            ...
            <ul>
              <li><b>Active</b> - Members who are engaged and participating in any aspect of the community and/or its projects. Activities vary broadly from meeting attendance to helping other contributors to using and providing feedback on projects, to code contribution, and so on.</li>
            ...
            <p>
              ...
              <ul>
                <li>Activity Badges are the badges that are assigned when a user completes a task like sharing a design, creating a design for the first time, etc.</li>
              </ul>
              <ul>
                <li>Project Badges are assigned to the user when a user contributes to a particular project.</li>
              </ul>
