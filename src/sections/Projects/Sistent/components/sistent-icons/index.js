...
const IconCard = ({ icon, isDark }) => {
  ...
  const handleIconClick = async (iconName) => {
    const importStatement = `import { ${iconName} } from "@layer5/sistent";`;
    try {
  ...
  return (
    <CustomTooltip
      title={
        <div className={`tooltip-content ${isDark ? "dark" : "light"}`}>
          <div
            className={`import-statement ${isDark ? "dark" : "light"}`}
          >
            {
              showPopup ? "Copied" : `import { ${icon.name} } from "@layer5/sistent";`
            }
          </div>
        </div>
      }
  ...
  return (
    <SistentLayout title="Sistent Icons">
      ...
      <div className="main-content">
        ...
        <p>Click on the icon to copy it to your clipboard. </p>
