...
const Paginate = ({ pageContext, isListView }) => {
  const [currentPage, setCurrentPage] = useState(pageContext.humanPageNumber || 1);
  ...
  return (
    <PaginateWrapper>
      <div className="btn-container">
        {previousPagePath && (
          <Link to={previousPagePath} className="btn" state={{ isListView }}>
            <IoIosArrowRoundBack />
          </Link>
        )}
        {pageNumbers.map((number) => (
          ...
          >
            <button
              ...
              className={number === currentPage ? "btn active-btn" : "btn page-btn"}
            >
              {number}
            </button>
          </Link>
        ...
        {nextPagePath && (
          <Link to={nextPagePath} className="btn" state={{ isListView }}>
