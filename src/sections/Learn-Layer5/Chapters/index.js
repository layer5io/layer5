...
const Chapters = ({ chapterData, courseData, location, serviceMeshesList, TOCData }) => {
  ...
  const tableOfContents = TOCData.filter((node) => !!node.fields.section).map((toc) => ({
    section: toc.fields.section,
    chapter: toc.fields.chapter
  }));
        ...
        <>
          ...
          >
            <Link
              to={`/${sm.slug}`}
              data-for="mesh-name"
              data-tip={capitalize(sm.section)}
              className="course"
              key={index}
            >
              <Image
