...
export const PricingAddons = ({
  ...
  useEffect(() => {
    if (selectedAddon) {
      ...
      if (selectedAddon.id === "academy") {
        const theorySubAddon = selectedAddon.subAddOns?.find(
          (sub) => sub.id === "academy-theory",
        );
      ...
      if (selectedAddon?.id === "academy" && selectedAddon.subAddOns) {
        selectedAddon.subAddOns.forEach((subAddOn) => {
          if (
            ...
            const subAddOnPricing =
              subAddOn.pricing && subAddOn.pricing[quantityIndex];
            if (subAddOnPricing) {
              const subAddOnPerUserCost = isYearly
                ? subAddOnPricing.yearlyPerUser
                : subAddOnPricing.monthlyPerUser;
      ...
      const enterpriseUsersCost =
        (isYearly ? enterprisePlan.yearlyprice : enterprisePlan.monthlyprice) *
        (enterpriseUsers > 0 ? enterpriseUsers : 1);
  ...
  const getPlanLinkForAcademy = () => {
    ...
    if (selectedSubAddOns["academy-practical"]) {
      targetSubAddon = selectedAddon.subAddOns?.find(
        (sub) => sub.id === "academy-practical",
      );
    } else {
      targetSubAddon = selectedAddon.subAddOns?.find(
        (sub) => sub.id === "academy-theory",
    ...
    const matchingPlanLink = targetSubAddon.planLink.find(
      (plan) =>
        plan.cadence === currentCadence &&
        plan.learners === currentLearnerCount,
    );
                ...
                title={
                  <Box sx={boxStyles.cardHeaderTitle}>
                    <Typography
                      ...
                      component="h3"
                      fontWeight="bold"
                      sx={typographyStyles.headerTitle}
                    >
                      Enterprise Add-ons
                    </Typography>
                ...
                <Box sx={boxStyles.cardContentInner}>
                  <Box>
                    <FormControl fullWidth>
                      <InputLabel sx={typographyStyles.qanelasFont}>
                      ...
                      <Box className="pricing-features">
                        ...
                        >
                          <FormControlLabel
                            ...
                            control={
                              <Switch
                                disabled
                                checked={
                                  selectedSubAddOns["academy-theory"] || false
                                }
                                onChange={(e) =>
                                  handleSubAddOnToggle(
                                    "academy-theory",
                                    e.target.checked,
                                  )
                                }
                          ...
                          >
                            ...
                            >
                              {selectedAddon?.subAddOns
                                ?.find((sub) => sub.id === "academy-theory")
                        ...
                        <Box
                          className="feature"
                          sx={boxStyles.featureContainerEnd}
                        >
                          <FormControlLabel
                            ...
                            control={
                              <Switch
                                ...
                                checked={
                                  selectedSubAddOns["academy-practical"] ||
                                  false
                                }
                                onChange={(e) =>
                                  handleSubAddOnToggle(
                          ...
                          >
                            <Box sx={boxStyles.featureChipsPractical}>
                              {selectedAddon?.subAddOns
                                ?.find((sub) => sub.id === "academy-practical")
                                ?.features?.map((feature, index) => (
                                  <Chip
                                    key={`practical-${index}`}
                                    icon={<CheckCircle />}
                      ...
                      <Box sx={boxStyles.learnerSection}>
                        <Slider
                          ...
                          valueLabelFormat={(value) => {
                            ...
                            if (
                              targetSubAddon?.pricing &&
                              targetSubAddon.pricing[value]
                            ) {
                              const option = targetSubAddon.pricing[value];
                              const pricePerUser = isYearly
                                ? option.yearlyPerUser
                                : option.monthlyPerUser;
                              const multiplier = selectedSubAddOns[
                                "academy-practical"
                              ]
                                ? 2
                                : 1;
                              const totalPrice =
                                pricePerUser * option.learners * multiplier;
                            ...
                            return (
                              targetSubAddon?.pricing?.map((option, index) => ({
                                ...
                                label: (
                                  ...
                                  >
                                    <Box>{formatLearners(option.learners)}</Box>{" "}
                                    {/* Changed from ternary */}
                                    <Box
                                      sx={{
                                        color: "text.secondary",
                                        mb: 1.5,
                                    ...
                                    >
                                      ...
                                      <br />
                                      {targetSubAddon.unitLabelSingular}/
                                      {isYearly ? "year" : "month"}
                                    </Box>
                                  </Box>
                                ),
                        ...
                        <Box sx={boxStyles.disclaimerSection}>
                          <Typography
                            variant="body2"
                            sx={typographyStyles.italic}
                          >
                            Looking for a plan larger than 2,500 learners?
                            Great! <a href="/company/contact">Let us know</a>.
                          </Typography>
                        </Box>
                      </Box>
                    </>
                        ...
                        <Slider
                          ...
                          valueLabelFormat={(value) => {
                            if (
                              ...
                              selectedAddon.pricing[value]
                            ) {
                              const option = selectedAddon.pricing[value];
                              const unitPrice = isYearly
                                ? option.yearlyPerUnit
                                : option.monthlyPerUnit;
                          ...
                          marks={
                            selectedAddon?.pricing?.map((option, index) => ({
                              value: index,
                              label: (
                                <Box sx={boxStyles.sliderMarks}>
                                  ...
                                  <Box sx={boxStyles.sliderPriceText}>
                                    {formatSliderPrice(
                                      isYearly
                                        ? option.yearlyPerUnit * option.units
                                        : option.monthlyPerUnit * option.units,
                                      currency,
                                    )}
                                  </Box>
                                </Box>
          ...
          {selectedAddon && (
            <Box>
              <Paper elevation={1} sx={boxStyles.pricingPaper}>
                <Box
                  sx={{ ...boxStyles.flexBetween, ...boxStyles.pricingHeader }}
                >
                  <Typography
                    variant="h6"
                    sx={typographyStyles.subheading}
                    gutterBottom
                  >
                ...
                <Box
                  sx={{ ...boxStyles.flexColumn, ...boxStyles.pricingItems }}
                >
                  <Box sx={boxStyles.flexBetween}>
                    <Typography
                      variant="body1"
                      sx={typographyStyles.pricingItemLeft}
                    >
                      {selectedAddon?.id === "academy"
                    ...
                    <Typography
                      variant="body1"
                      fontWeight="500"
                      sx={typographyStyles.pricingItemRight}
                    >
                      {(() => {
                        if (selectedAddon?.id === "academy") {
                          const theorySubAddon = selectedAddon?.subAddOns?.find(
                          ...
                          if (
                            theorySubAddon?.pricing &&
                            theorySubAddon.pricing[quantityIndex]
                          ) {
                            const currentLearnerOption =
                              theorySubAddon.pricing[quantityIndex];
                            const monthlyPerUserCost =
                              currentLearnerOption.monthlyPerUser;
                            const yearlyPerUserCost =
                              currentLearnerOption.yearlyPerUser;
                            const totalCost = isYearly
                              ? yearlyPerUserCost *
                                currentLearnerOption.learners
                              : monthlyPerUserCost *
                                currentLearnerOption.learners;
                          ...
                          if (
                            selectedAddon?.pricing &&
                            selectedAddon.pricing[quantityIndex]
                          ) {
                            const currentOption =
                              selectedAddon.pricing[quantityIndex];
                            const monthlyPerUnitCost =
                              currentOption.monthlyPerUnit;
                            const yearlyPerUnitCost =
                              currentOption.yearlyPerUnit;
                            const totalCost = isYearly
                              ? yearlyPerUnitCost * currentOption.units
                              : monthlyPerUnitCost * currentOption.units;
                            return formatPrice(totalCost);
                          ...
                          <Box key={subAddOn.id} sx={boxStyles.flexBetween}>
                            ...
                            >
                              {(() => {
                                const subAddOnPricing =
                                  subAddOn.pricing &&
                                  subAddOn.pricing[quantityIndex];
                                if (subAddOnPricing) {
                                  const subAddOnPerUserCost = isYearly
                                    ? subAddOnPricing.yearlyPerUser
                                    : subAddOnPricing.monthlyPerUser;
                  ...
                  <Box sx={boxStyles.flexBetween}>
                    ...
                    <Typography
                      variant="h4"
                      fontWeight="bold"
                      sx={typographyStyles.qanelasFont}
                    >
                      {formatPrice(totalPrice)}
                      <Typography
                        variant="h4"
                        component="span"
                        style={boxStyles.priceComponent}
                      >
                        /{isYearly ? "yearly" : "monthly"}
                      </Typography>
                    </Typography>
                  ...
                  <Box sx={boxStyles.buttonSection}>
                    <Button
                      ...
                      $url={
                        selectedAddon?.id === "academy"
                          ? getPlanLinkForAcademy().link
                          : getPlanLinkForOtherAddons().link
                      }
                    >
                      {selectedAddon?.id === "academy"
                        ? getPlanLinkForAcademy().name
                        : getPlanLinkForOtherAddons().name}
                    </Button>
                  </Box>
                </Box>

                <Typography sx={typographyStyles.priceDisclaimer}>
                  *Prices shown are {isYearly ? "annual" : "monthly"}{" "}
                  subscription costs.{" "}
                  {isYearly
                    ? "Monthly subscriptions are available at standard rates."
