                ...
                if (
                  ...
                  return (
                    <th key={column} {...column.getHeaderProps()}>
                      ...
                      <div style={{ fontSize: "0.9rem", fontWeight: "500" }}>{spec[column.id]}</div>
                    </th>
                  );
                } else {
                  return (
                    ...
                    {non_functional.find((ele) => ele.name.includes(row.original.mesh_name)) ? (
                      <>
                        <img
                          ...
                          data-tip={row.original.mesh_name}
                          className="smiMark"
                          src={
                            non_functional.find((ele) => ele.name.includes(row.original.mesh_name))
                              .icon
                      ...
                      return (
                        <td key={`spec${i}`}>
                          <div
                            className="tooltip-div"
                            data-for="react-tooltip"
                            data-tip={`${spec["reason"]}<br>${spec["result"]}`}
                          >
                ...
                {row.original.previous_versions &&
                  row.original.previous_versions.map((prevResult) => {
                    return (
                      <tr
                        key={`collapse-row${i}`}
                        className={isCollapsed[i] ? "secondaryRow" : "secondaryRow-hidden"}
                      >
                        <td></td>
