---
title: "An Introduction to Contributing to Meshery"
thumbnail: ./intro-to-meshery.png
category: Service Mesh
tags: 
  - Meshery
  - Tutorial
author: Vijay Cherukuri
type: Tutorial
product: Meshery
featured: false
published: true
resource: true
---

import { ResourcesWrapper } from "../../Resources.style.js";

<ResourcesWrapper> 
        <iframe	 src="https://docs.google.com/presentation/d/1Wc5ALdn-G3fADJ8I6nJlyOGhV2XHVprYPqDvkZ1MEqY/embed?start=false&loop=false&delayms=3000"	
              frameBorder="0" allowFullScreen width="100%" height="500">	
        </iframe>
      
        <iframe className="iframe" src="https://www.youtube.com/embed/67iy2JEp4Ss"
              frameBorder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"allowFullScreen width="100%" height="500">
        </iframe>
</ResourcesWrapper>