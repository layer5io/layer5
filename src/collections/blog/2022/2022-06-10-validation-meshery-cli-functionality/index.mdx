...
<pre><code className="language-bash">gh pr checkout https://github.com/meshery/meshery/pull/4823</code></pre></li>
<li><p>You can check if you&#39;re into the same branch as the PR author with </p><pre><code className="language-bash">git branch</code></pre></li>
<li><p>Well, if we&#39;re testing a PR related to mesheryctl, we need to build the binary from the same branch. change your directory to <code>mesheryctl</code> folder and run</p><pre><code className="language-bash">make</code></pre><p>This will create a mesheryctl binary according to your OS in the same directory</p></li>
