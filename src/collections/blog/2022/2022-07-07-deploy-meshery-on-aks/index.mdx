...
<div class="intro">
  <p>
    <a href="https://meshery.io/">Meshery</a>'s goal is to make the operation of cloud native
...
<p>
  ...
  following command creates a resource group named MesheryGroup in <code>southindia</code> location.{" "}
</p>
<pre>
  <code class="language-bash">az group create --name MesheryGroup --location southindia</code>
</pre>

<p>
  Create AKS cluster using <code>az aks create</code>. The following command creates aks cluster
  with a single node.{" "}
</p>
...
<p>
  You can connect with your cluster by using <code>az aks get-credentials</code> , which basically
  downloads credentials and configure the Kubernetes CLI.{" "}
</p>
<pre>
  <code>az aks get-credentials --resource-group MesheryGroup --name MesheryAKS</code>
</pre>
<p>
...
<p>
  Meshery server supports customizing authentication flow callback URL, which can be configured in
  the following way.
</p>
...
<p>
  ...
  <a href="https://layer5.io/community">community</a> and share your deployment’s configuration on
  the <a href="https://discuss.layer5.io/"> discussion forum </a>today!{" "}
</p>
