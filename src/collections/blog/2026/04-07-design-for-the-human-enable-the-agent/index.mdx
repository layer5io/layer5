...
description: "CLIs are human interfaces, not APIs - but scriptability has always mattered. Here's how mesheryctl is being redesigned to serve both humans and AI agents."
type: Blog
category: AI
tags:
  - ai
...
There's a lively debate in the industry right now about whether the Model Context Protocol (MCP) is the right abstraction for AI-agent-to-tool integration. MCP adoption has been rapid - [OpenAI shipped full MCP support in ChatGPT](https://www.infoq.com/news/2025/10/chat-gpt-mcp/), and [Google DeepMind committed to supporting it across Gemini](https://techcrunch.com/2025/04/09/google-says-itll-embrace-anthropics-standard-for-connecting-ai-models-to-data/). The momentum is real.
...
Where MCP can stumble is efficiency. Independent benchmarks comparing CLI and MCP interfaces for equivalent agent tasks have found [MCP connections failing at meaningfully higher rates than their CLI counterparts](https://www.scalekit.com/blog/mcp-vs-cli-use), and verbose tool-definition schemas eating into the context budget an agent needs for reasoning. CLI interactions are leaner by construction: the agent calls a command, reads stdout, moves on.
