...
<p>
To subsribe to a specific release channel or version using mesheryctl you can use 
...
<p>
  Meshconfig is a configuration file that is used to configure Meshery. It is typically located in the <code>~/.meshery/config.yaml</code> directory.It contains information about the current release channel, the version of Meshery that isinstalled, and other configuration options that are specific to your Meshery installation. 
...
<li> To view the newly created meshery context use <pre><code class="language-bash">mesheryctl system context view [contex-name]</code></pre></li>
