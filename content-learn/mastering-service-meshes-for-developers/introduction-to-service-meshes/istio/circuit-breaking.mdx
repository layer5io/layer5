...
Before we can configure circuit breaking, please try to access the `product page` app from within `Meshery` to ensure all the calls are making it through **without** errors as we did in [Observability chapter](/learn/learning-paths/mastering-meshery-istio/introduction-to-service-meshes/istio/observability) (<a href={MesheryInitialLoadTest}>see screenshot</a>).
